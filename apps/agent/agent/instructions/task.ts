			...
			const { markdown, focus } = await sessionPreamble(
				{
					...
					matterId: attributeText.parse(attributes.matterId),
				},
				{
...
export function builderTaskMarkdown(
	...
	const task =
		commandType === "CREATE_AGENT"
			...
			: `This is a private CRM assistant chat. Answer the user's question directly. Use tagged records as scope and use available read-only CRM and research tools when evidence is needed. Use list_matters for pipeline-wide, open-matter, or inactivity questions and follow its pagination until the requested scope is complete. The chat renders list_matters output as a structured matter list. Do not restate or enumerate individual matter rows in prose, bullets, or tables; the structured list is the sole row-level presentation. Give only a concise synthesis, caveats, and useful next actions after the tool results. If one materially necessary decision is missing, call ask_question with one focused follow-up instead of guessing; do not interrupt for optional detail. Do not call agent_builder, create an agent draft, or mutate CRM records on this turn. Agent creation begins only from an explicit request to create or build one. Be concise, distinguish CRM evidence from inference, and say when the CRM does not contain the answer.`;
