...
function explain(error: string): string {
	switch (error) {
		case "no_user_grant":
			return "This workspace did not grant MyLegalXpert permission to add itself to a private channel.";
		case "channel_not_found":
			return "Slack cannot see this channel. A member has to invite MyLegalXpert.";
		case "is_archived":
			return "This channel is archived. Somebody has to unarchive it before MyLegalXpert can join.";
		case "missing_scope":
		...
		case "unknown_bot_user":
			return "Slack did not report which user MyLegalXpert is.";
		default:
...
export async function createSlackChannel(
	...
	if (!token) {
		return {
			error: isPrivate
				? "This workspace did not grant MyLegalXpert permission to create a private channel."
