...
describe("agent builder commands", () => {
	it("enters agent creation for slash commands and direct build requests", () => {
		expect(builderCommandType("/Create agent Flag stalled matters")).toBe(
			"CREATE_AGENT",
		);
		...
		expect(
			builderCommandType('Create an agent named "Stalled Matter Brief".'),
		).toBe("CREATE_AGENT");
		expect(builderCommandType("Build and save a new agent for renewals")).toBe(
			"CREATE_AGENT",
		);
		expect(builderCommandType("Build me an agent for stalled matters")).toBe(
			"CREATE_AGENT",
		);
		expect(builderCommandType("Build the agent I described above")).toBe(
			"CREATE_AGENT",
		);
		expect(builderCommandType("Please set up an agent for overdue matters")).toBe(
			"CREATE_AGENT",
		);
		expect(
		...
		expect(consumeBuilderCommand("/Create agent Flag stalled matters")).toEqual({
			commandType: "CREATE_AGENT",
			body: "Flag stalled matters",
		});
		expect(
			consumeBuilderIntent("Build an agent that flags stalled matters"),
		).toEqual({
			commandType: "CREATE_AGENT",
			body: "that flags stalled matters",
